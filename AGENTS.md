...
Run once after cloning to install the local git pre-push hook:

```bash
./scripts/install-hooks.sh
```

The hook runs `dart analyze --fatal-infos` before every push and blocks the push if there are any warnings or infos.
